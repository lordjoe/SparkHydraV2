...
//    }
    ...
    private static void showTeenAgersCount(final JavaSparkContext sc, final String name) {
        ...
        Dataset<Row> parquetFile = sqlContext.parquetFile(parquetName);

         //Parquet files can also be registered as tables and then used in SQL statements.
         parquetFile.createOrReplaceTempView(name);
        Dataset<Row> teenagers = sqlContext.sql("SELECT COUNT(*) FROM " + name + "  WHERE age >= 13 AND age <= 19");
    ...
    private static void showTeenAgers(final JavaSparkContext sc, final String name) {
        ...
        //Parquet files can also be registered as tables and then used in SQL statements.
        parquetFile.createOrReplaceTempView(name);
        Dataset<Row>  teenagers = sqlContext.sql("SELECT name FROM "  + name + "  WHERE age >= 13 AND age <= 19");
        List<String> teenagerNames = teenagers.toJavaRDD().map(new Function<Row, String>() {
            public String call(Row row) {
                String name = row.getString(0);
                return "Name: " + name;
    ...
    public static void buildParaquetDatabase(JavaSparkContext sc , String name) {
        try {
            ...
            schemaPeople.write().parquet(parquetName);
        }
        catch (IOException e) {
