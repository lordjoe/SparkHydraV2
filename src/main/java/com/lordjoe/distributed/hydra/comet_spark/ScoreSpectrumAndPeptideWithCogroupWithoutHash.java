...
public class ScoreSpectrumAndPeptideWithCogroupWithoutHash extends AbstractLoggingFlatMapFunction<Tuple2<BinChargeKey, Tuple2<Iterable<CometScoredScan>, Iterable<IPolypeptide>>>, IScoredScan> {
    ...
    private  MemoryUseAccumulator  memoryAccululator;
    ...
    private final LongAccumulator numberScoredAccumlator = AccumulatorUtilities.getInstance().createAccumulator(CometScoringHandler.TOTAL_SCORRED_ACCUMULATOR_NAME);
    private final LongAccumulator numberSpectraAccumlator = AccumulatorUtilities.getInstance().createAccumulator(CometScoringHandler.TOTAL_SCORED_SPECTRA_NAME);


    private final  CountedDistribution  peptideDistributionCounts = (CountedDistribution)AccumulatorUtilities.getInstance().createSpecialAccumulator(CometScoringHandler.PEPTIDES_ACCUMULATOR_NAME,
              CountedDistribution.empty());
    private final  CountedDistribution  spectrumDistributionCounts = (CountedDistribution)AccumulatorUtilities.getInstance().createSpecialAccumulator(CometScoringHandler.SPECTRA_ACCUMULATOR_NAME,
              CountedDistribution.empty());
    private final MemoryUseAccumulatorAndBinSize binAccululator = (MemoryUseAccumulatorAndBinSize)AccumulatorUtilities.getInstance().createSpecialAccumulator(MemoryUseAccumulatorAndBinSize.BIN_ACCUMULATOR_NAME,
...
//                CometTemporaryMemoryAccumulator.PARAM_INSTANCE, CometTemporaryMemoryAccumulator.empty());
    private final NotScoredBins unscoredAccumulator = (NotScoredBins)AccumulatorUtilities.getInstance().createSpecialAccumulator(NotScoredBins.BIN_ACCUMULATOR_NAME,
             NotScoredBins.empty());
    private final LogRareEventsAccumulator rareEvents =
            (LogRareEventsAccumulator)AccumulatorUtilities.getInstance().getSpecialAccumulator(LogRareEventsAccumulator.LOG_RARE_EVENTS_NAME);
    ...
    public ScoreSpectrumAndPeptideWithCogroupWithoutHash(XTandemMain application) {
        ...
        memoryAccululator = ( MemoryUseAccumulator ) instance.getSpecialAccumulator(MemoryUseAccumulator.MEMORY_ACCUMULATOR_NAME);
    ...
    @Override
    public Iterator<IScoredScan> doCall(Tuple2<BinChargeKey, Tuple2<Iterable<CometScoredScan>, Iterable<IPolypeptide>>> inp) throws Exception {
